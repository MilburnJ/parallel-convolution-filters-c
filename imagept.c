...
void* useThreads(void * argument_ptr) {
    struct conv * args = (struct conv *) argument_ptr;
    Image* srcImage = args->srcImage;
    Image* destImage = args->destImage;
    int threadNum = args->rank;
    int row,pix,bit,span;
    span=srcImage->bpp*srcImage->bpp;
    for (row=threadNum; row<srcImage->height; row += thread_count){
...
void convolute(Image* srcImage,Image* destImage,Matrix algorithm){
    ...
    for(i = 0; i < thread_count; i++) {
        struct conv args = {i, srcImage, destImage};
        pthread_create(&thread_array[i], NULL, &useThreads, (void*) &args);
    }
    for(i = 0; i < thread_count; i++) {
